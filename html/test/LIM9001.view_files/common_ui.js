...
//				lootPoint.children().children().children().children().children("input").prop("checked",true);
				...
				lootPoint.find(".subClsY").prop("checked", true);
				$(this).parent().parent().next(".meta-chk").children().children().children("input").prop("checked", true);
				lootPoint.next().find(".subClsY").prop("checked",true);
				lootPoint.children().children().children().children().children("input").prop("checked", true);
				...
				/*s : 2023-07-04*/
				 //스크롤 발생 이벤트 처리
				$(".popup-layout-full>dd").scroll(function(){
				   var scrollTop = $(this).scrollTop();
					var innerHeight = $(this).innerHeight();
				   var scrollHeight = $(this).prop("scrollHeight");
					if (scrollTop + innerHeight >= scrollHeight - 10) {  // 스크롤바가 아래 쪽에 위치할 때
						$(".btn-ok").addClass("popok");
					}else{
			...
			*/
			$('#all_check').click(function(){
				...
				var termSize = $(".isNecessary").size();
				/* 2020.09.08 전체동의시 전체약관보이기  -- */
				
				if($(this).is(":checked")){
					$('.termsY').prop("checked",true);
					
					$('.subCls').prop("checked",true); /*서브약관 체크 동의*/
					
					/* -- 2020.09.08 전체동의시 전체약관보이기 */
					for(var idx = termSize; idx>=0; idx--){
						termEq = $(".isNecessary").eq(idx).attr('data-tab');
						$("#"+termEq).show();
					}
					/* 2020.09.08 전체동의시 전체약관보이기  -- */
					...
					$('.termsN').prop("checked",true);
					
					$('.subCls').prop("checked",false); /*서브약관 체크 동의*/
...
/* 스마트 저축은행 */
	...
	function modalOff() { // 모달창 닫기
		$(".popup-layout").hide();
		$(".popup-layout-full").hide();
		$(".scr-modal-agreeImg>dd").scrollTo(0,0);
	};

	function modalOff18() { // 모달창 닫기
		modalOff();
		
		$(this).parents("li").find(".agg_btn .subClsNN").prop("checked", true);
	};
...
///////////////////////////////////////////////////////////////	
	$(".all #all_check").click(function() { // 약관 전체 동의 클릭 시 약관 모아 보여 주기
		agreeAllView();
...
function agreeAllView() { // 모달 쇼 -> 이미지 삭제 -> btn a data 수집 -> 이미지 추가
	...
	$(".btn a").each(function(index, item) {
...
//	$('#unchk>a').click(function(){
//		var tab_id = $(this).attr('data-tab');
//		$(".scr-modal-agreeImg18").show();
//		$(".scr-modal-agreeImg18 .agree-img>img").remove();
//		$(".agree-img").css("width","100%");
//		$(".agree-img").append("<img src='../../WEB/images/K_cert/" + tab_id + ".png' />");
//	});
//	
//	$('.scr-modal-agreeImg18 .btn-ok').click(function(){
//		$("popup-layout-full").hide();
//		
//		$(".agg_btn .subClsNN").prop("checked", true);
//		
//	});
