	...
	*/
	...
	$('.termsE_Y').click(function(){
		...
		if($(this).prop("checked")){
			...
			$("#view_YN").val(viwe_id);
			/*$(".class3").scrollHeight - $(".class3").clientHeight > 0{
				alert("Y축 스크롤바 생성");
			}*/
			//document.querySelector(".scr-modal-agreeImg>dd").scrollTo(0,0);
			 $(".scr-modal-agreeImg>dd").prop({ scrollTop: 0 });
			return false;
	...
	$('.subClsT').click(function(){
		if($(this).prop("checked")){
			termsAndAgreeE('all_check', 'subClsT');
			var confirm_id = $(this).attr('data-id');
			$("#cntl_YN").val(confirm_id);
			var viwe_id = $(this).attr('data-tab');
			var btn_hide = $('.btn-ok').addClass('hide');
